# V2bX-Nodemix# V2bX-Nodemix



> V2bX 多实例管理系统 - 支持 WireGuard 出站 + Web 管理界面> 基于 sing-box 的 V2bX 多实例管理系统



一台服务器运行多个 V2bX 实例,每个实例通过 sing-box 配置独立的 WireGuard 出口节点。在单台服务器上运行多个 V2bX 实例，每个实例通过 sing-box 内置的 WireGuard 使用不同的出口节点。



## ✨ 核心功能[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)



- 🚀 多实例管理 - 单服务器运行多个 V2bX 节点## ✨ 特性

- 🔐 WireGuard 出站 - 每个实例独立 WireGuard 配置

- 🌐 Web 管理界面 - 在线管理实例和配置- 🚀 **多实例管理** - 一台服务器运行多个 V2bX 实例

- 🎮 交互式控制台 - 17项管理功能- 🔐 **WireGuard 出站** - 在 `sing_origin.json` 中配置 WireGuard

- 🔄 批量操作 - 一键启动/停止所有实例- 🌐 **Web 管理界面** - 在线编辑配置文件

- 🚦 开机自启 - systemd 服务管理- ⚡ **简单高效** - 无需系统级 WireGuard 接口

- 📦 一键安装 - 自动处理所有依赖- 🔌 **插件式设计** - 可作为 V2bX 的扩展插件使用
...
wget -O install.sh https://raw.githubusercontent.com/Cd1s/V2bX-Nodemix/main/install.sh├── v2bx-manager.sh             # 实例管理脚本

chmod +x install.sh├── web/                        # Web 管理界面

sudo bash install.sh│   ├── app.py                 # Flask 后端

```│   ├── start-web.sh           # 启动脚本

│   └── templates/             # HTML 模板

安装完成后运行:├── configs/                    # 配置文件目录

```bash│   ├── node1/                 # 实例1

v2bx-nodemix│   │   ├── config.json        # V2bX 配置
...
v2bx-nodemix start <实例名>      # 启动实例## 🚀 快速开始

v2bx-nodemix stop <实例名>       # 停止实例

v2bx-nodemix restart <实例名>    # 重启实例### 方式一：一键安装（推荐）
...
自启管理:vi /opt/V2bX-Nodemix/configs/example/sing_origin.json
  ...
  12 ❌ 关闭开机自启# 2. 生成 WireGuard 密钥（如需要）
...
系统管理:# 3. 进入管理控制台
  ...
  14 🔐 修改 Web 密码v2bx-nodemix

  15 🌐 Web 服务管理

  16 🔄 升级 V2bX-Nodemix# 或使用命令行

  17 🗑️  卸载 V2bX-Nodemixv2bx-nodemix start example   # 启动实例
