...

{
  ...
  "Nodes": [{
    ...
    "Core": "sing",访问: `http://服务器IP:5000`chmod +x install.sh├── web/                        # Web 管理界面
    ...
    "NodeType": "vless",默认密码: `admin123`sudo bash install.sh│   ├── app.py                 # Flask 后端
...
### sing_origin.json- 📊 实例状态监控



```json- ⚙️ 在线编辑配置文件│   └── templates/             # HTML 模板

{

  "outbounds": [- 🎛️ 启动/停止/重启实例
    ...
    {- 📋 查看实时日志安装完成后运行:├── configs/                    # 配置文件目录
      ...
      "server_port": 51820,- 🗑️ 删除实例```bash│   ├── node1/                 # 实例1
      ...
      "local_address": ["10.0.1.2/16"]修改密码: 运行 `v2bx-nodemix` 选择 `14) 修改 Web 密码`v2bx-nodemix│   │   ├── config.json        # V2bX 配置
  ...
  "route": {
    ...
    "rules": [---```│   │   └── sing_origin.json   # sing-box 配置
...
```bash│   ├── config.json          # V2bX 配置

v2bx-nodemix

# 选择 16) 升级 V2bX-Nodemix│   └── sing_origin.json     # sing-box 配置 (WireGuard 出站)    ├── node1/
...
sudo bash update.sh└── template/                # 配置模板
...
# 选择 17) 卸载 V2bX-Nodemix```json# 启动交互式管理界面    └── node2/
...
```bash  "Log": { "Level": "info" },v2bx-nodemix        └── ...

systemctl stop v2bx-nodemix-web v2bx-nodemix-instances

rm -rf /opt/V2bX-Nodemix  "Cores": [{

rm -f /usr/local/bin/v2bx-nodemix

rm -f /etc/systemd/system/v2bx-nodemix-*.service    "Type": "sing",```
...
MIT License    "Core": "sing",v2bx-nodemix start <实例名>      # 启动实例## 🚀 快速开始
